...
def create_category(payload: CategoryCreateSchema, category_service: CategoryService = Depends(get_category_service)):
    return category_service.create_category(payload)

@router.patch('/{category_id}', response_model=CategorySchema)
def update_category(category_id: str, payload: CategoryUdpateSchema, category_service: CategoryService = Depends(get_category_service)):
